...
FastJetProcessor::FastJetProcessor() : Processor("myFastJetProcessor")
{
...
void FastJetProcessor::processEvent(LCEvent * evt)
{
	...
	if (_clusterMode == FJ_exclusive_nJets && jets.size() == _nrJets)
	{
		...
		lccJetParams.setValue(string("d_{n-1,n}"), (float)cs.exclusive_dmerge_max(nrJets-1));
		lccJetParams.setValue(string("d_{n,n+1}"), (float)cs.exclusive_dmerge_max(nrJets));

		lccJetParams.setValue(string("y_{n-1,n}"), (float)cs.exclusive_ymerge_max(nrJets-1));
		lccJetParams.setValue(string("y_{n,n+1}"), (float)cs.exclusive_ymerge_max(nrJets));
	}
